...
class DeviceModel extends Model
{
	...
	protected function onBeforeInsert($data)
	{
		...
		// check if PMUX device already exists in porch
		if (4 == $data['data']['type']) {
			$result = $this->where(['type' => 4, 'porch_id' => $data['data']['porch_id']])->first();
			if (null !== $result) {
				throw new \Exception('PMUX_ALREADY_EXISTS');
			}
		}
	...
	protected function onAfterInsert($data)
	{
		...
		if (!array_key_exists('building_id', $data['data']))
			$data['data']['building_id'] = null;
		...
		if (9 == $data['data']['type']) {
			...
			// regular SIP-client
			// offset username by 100000 to make sipusername start from 100000 but 0
			$sipusername = sprintf('%06d', $data['id'] + 100000);
		}

		if (null !== $data['data']['sippassword']) {
			$sippassword = $data['data']['sippassword'];
		} else {
		...
		if (9 == $data['data']['type']) {
			...
			foreach ($rooms as $room) {
				...
				$this->insert([
					...
					"type" => 0,
					"sippassword" => $sippassword,
					"comment" => "аб. PMUX-трубка, ул. " . $street['title'] . " д. " . $building['num'] . " под." . $porch['num'] . " кв. " . $room['num']
				], false);
			}
	...
	protected function onAfterUpdate($data)
	{
		...
		foreach ($data['id'] as $id) {
			...
			// update all slave PMUX devices when updating master
			if (4 == $device['type']) {
				$devices = $this->where(["porch_id =" => $device['porch_id'], 'type =' => 0])->doFindAll();
				foreach ($devices as $dev) {
					$fields = [];
					if (null !== $data['data']['porch_id'])
						$fields['porch_id'] = $data['data']['porch_id'];

					if (null !== $data['data']['sippassword'])
						$fields['sippassword'] = $data['data']['sippassword'];


					$this->update($dev['id'], $fields);
				}
			}
	...
	protected function onBeforeDelete($data)
	{
		...
		foreach ($data['id'] as $id) {
			...
			// delete all pmux slave devices when deleting pmux master
			if (4 == $device['type']) {
				$result = $this->get(["device.type =" => 0, "device.porch_id =" => $device['porch_id']]);
				$pmuxDevices = $result->rows;
				foreach ($pmuxDevices as $dev) {
					$this->delete($dev['id']);
				}
			}

			// if (0 == $device['type']) {
			// 	throw new \Exception('PMUX_SLAVE_DELETE_PERMITTED');
			// }
