...
class Buildings extends BaseController {
  ...
  function postSave() {
	...
	// check if num contains more than one letter or any special character
	if (preg_match("/[a-zA-Zа-яА-Я]{3,}|[!@#$%№^&*()_+\-=\[\]{};':\"\\|,.<>\/?]+/", $num)) {
		return json_encode([
			'result' => false,
			'message' => 'Неккоректный номер дома ' . $num
		]);
	}
	...
	$last_char = mb_substr($num, -1);
	
	$code = "$num";

	if (!is_numeric($last_char)) {
		// remove letter character from code
		$code = mb_substr($code, 0, -1);
		// make code 000 format end leave space for letter identifier
		while (strlen($code) < 3)
			$code = "0$code";
		
		// concatinate adjacent number with this letter to code 
		$code = $code . $map_table[$last_char];
	} else {
		// check if we have letter infront or in the middle of the num
		if (!is_numeric($code))
			return json_encode([
				'result' => false,
				'message' => 'Неккоректный номер дома ' . $num
		...
		// else just make right format for code
		while (strlen($code) < 3)
			$code = "0$code";
	
		// if after all code is still less than 4 characters long, push rightmost zero to it
		if (strlen($code) < 4)
			$code = "{$code}0";
