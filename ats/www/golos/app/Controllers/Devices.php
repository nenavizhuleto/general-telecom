...
class Devices extends BaseController
{
	public function getIndex()
	{
		...
		foreach ($result->rows as &$row) {
			switch ($row['type']) {
				case 0:
					$row['type_name'] = 'абонент, PMUX-трубка';
					break;
				case 1:
	...
	function postSave()
	{
		...
		try {
			...
			switch ($e->getMessage()) {
				...
				case 'PMUX_ALREADY_EXISTS':
					$message = 'Подъезд уже имеет PMUX устройство.';
					break;

				default:
					$message = 'Ошибка создания устройства.' . $e->getMessage();
					break;
			}
	...
	function postDelete()
	{
		...
		try {
			...
			switch ($e->getMessage()) {
				case 'PMUX_SLAVE_DELETE_PERMITTED':
					$result['message'] = 'Нельзя удалить устройство типа PMUX-трубка отдельно от главного PMUX';
					break;
				default:
	...
	function postPmux()
	{
		...
		if (1 != (null !== $block_id) + (null !== $porch_id))
			return json_encode([
				'result' => false,
				'message' => 'Неверный запрос.'
			]);
